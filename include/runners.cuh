...
#include "../src/kernels/esmm_dual_sparsity.cu"
...
// ============================================================================
// DUAL-MATRIX SPARSITY GEMM (Kernel 20)
// ============================================================================

bool run_esmm_dual_sparsity(int rows, int cols, int inners, float *d_A, float *d_B,
                              float *d_C, float *h_C, float *h_C_ref, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  const uint BK = 8;
  const uint WN = 32;  // Same as K19
  const uint WM = 64;  // Same as K19
  const uint WNITER = 1;
  const uint TN = 1;   // Same as K19
  const uint TM = 8;   // Same as K19

  dim3 blockDim(NUM_THREADS);
  dim3 gridDim(CEIL_DIV(cols, BN), CEIL_DIV(rows, BM));
  cudaMemset(d_C, 0, rows * cols * sizeof(float));

  // Preprocess both A and B matrices
  DualPatternMetadata dual_meta = analyze_dual_sparsity_patterns_gpu(
      d_A, rows, inners, d_B, cols, WM, WN, BK);

  // Run kernel with dual sparsity patterns
  for (int i = 0; i < runs; i++) {
    esmm_dual_sparsity<BM, BN, BK, WM, WN, WNITER, TM, TN, NUM_THREADS>
        <<<gridDim, blockDim>>>(rows, cols, inners, d_A, d_B, d_C,
                                dual_meta.d_A_blockPatterns,
                                dual_meta.d_B_blockPatterns,
                                dual_meta.numKBlocks);
  }
  ...
  if (error != cudaSuccess) {
    printf("CUDA error: %s\n", cudaGetErrorString(error));
    free_dual_pattern_metadata(dual_meta);
    return false;
  }
...
bool run_esmm_dual_sparsity_no_check(int rows, int cols, int inners, float *d_A,
                                      float *d_B, float *d_C, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
...
// ============================================================================
// B-TRANSPOSE WITH ILP (Kernel 22)
// ============================================================================

#include "../src/kernels/esmm_btranspose_ilp.cu"

bool run_esmm_btranspose_ilp(int rows, int cols, int inners, float *d_A, float *d_B,
                              float *d_C, float *h_C, float *h_C_ref, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  const uint BK = 8;
  const uint WN = 64;  // Back to K17's layout
  const uint WM = 32;
  const uint WNITER = 4;  // Back to K17's layout
  const uint TN = 8;   // High ILP
  const uint TM = 1;
  ...
  // Preprocess B: transpose and analyze column-wise sparsity (now per WN=64 columns)
  BTPatternMetadata BT_meta = preprocess_b_transpose(d_B, inners, cols, WN, BK);

  for (int i = 0; i < runs; i++) {
    esmm_btranspose_ilp<BM, BN, BK, WM, WN, WNITER, TM, TN, NUM_THREADS>
        <<<gridDim, blockDim>>>(rows, cols, inners, d_A, d_B, d_C,
                                BT_meta.d_blockPatterns, BT_meta.numKBlocks);
  }
...
bool run_esmm_btranspose_ilp_no_check(int rows, int cols, int inners, float *d_A,
                                       float *d_B, float *d_C, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  ...
  // Preprocess B: transpose and analyze column-wise sparsity
  BTPatternMetadata BT_meta = preprocess_b_transpose(d_B, inners, cols, WN, BK);
...
// ============================================================================
// HIERARCHICAL DUAL SPARSITY (Kernel 23)
// ============================================================================

#include "../src/kernels/esmm_hierarchical.cu"

bool run_esmm_hierarchical(int rows, int cols, int inners, float *d_A, float *d_B,
                            float *d_C, float *h_C, float *h_C_ref, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  ...
  // Preprocess B matrix hierarchically
  BHierarchicalMetadata B_meta = analyze_b_hierarchical_gpu(d_B, inners, cols, BK, TN);

  for (int i = 0; i < runs; i++) {
    esmm_hierarchical<BM, BN, BK, WM, WN, WNITER, TM, TN, NUM_THREADS>
        <<<gridDim, blockDim>>>(rows, cols, inners, d_A, d_B, d_C,
                                A_meta.d_blockPatterns,
                                B_meta.d_coarse_patterns, B_meta.d_fine_patterns,
                                A_meta.numKBlocks, B_meta.numNBlocks_coarse, B_meta.numNBlocks_fine);
  }
  ...
  if (error != cudaSuccess) {
    printf("CUDA error: %s\n", cudaGetErrorString(error));
    free_block_pattern_metadata(A_meta);
    free_b_hierarchical_metadata(B_meta);
    return false;
  }
...
bool run_esmm_hierarchical_no_check(int rows, int cols, int inners, float *d_A,
                                     float *d_B, float *d_C, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
...
// ============================================================================
// B-TRANSPOSE WITH COALESCED WRITES + TRANSPOSE (Kernel 24)
// ============================================================================

#include "../src/kernels/esmm_btranspose_coalesced.cu"

bool run_esmm_btranspose_coalesced(int rows, int cols, int inners, float *d_A, float *d_B,
                                    float *d_C, float *h_C, float *h_C_ref, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  ...
  // Allocate temporary column-major C
  float *d_C_colmajor;
  cudaMalloc(&d_C_colmajor, rows * cols * sizeof(float));
  cudaMemset(d_C, 0, rows * cols * sizeof(float));
  cudaMemset(d_C_colmajor, 0, rows * cols * sizeof(float));

  // Preprocess B
  BTPatternMetadata BT_meta = preprocess_b_transpose(d_B, inners, cols, WN, BK);

  for (int i = 0; i < runs; i++) {
    // Run kernel (output to column-major)
    esmm_btranspose_coalesced<BM, BN, BK, WM, WN, WNITER, TM, TN, NUM_THREADS>
        <<<gridDim, blockDim>>>(rows, cols, inners, d_A, d_B, d_C_colmajor,
                                BT_meta.d_blockPatterns, BT_meta.numKBlocks);

    // Transpose back to row-major
    dim3 transGrid((cols + TILE_DIM - 1) / TILE_DIM, (rows + TILE_DIM - 1) / TILE_DIM);
    dim3 transBlock(TILE_DIM, BLOCK_ROWS);
    transpose_kernel<<<transGrid, transBlock>>>(d_C_colmajor, d_C, rows, cols);
  }
...
bool run_esmm_btranspose_coalesced_no_check(int rows, int cols, int inners, float *d_A,
                                             float *d_B, float *d_C, int runs) {
  const uint NUM_THREADS = 256;
  const uint BN = 128;
  const uint BM = 128;
  ...
  // Time kernel + transpose together
  auto start = std::chrono::high_resolution_clock::now();
  for (int i = 0; i < runs; i++) {
    cudaMemset(d_C_colmajor, 0, rows * cols * sizeof(float));
  ...
  std::chrono::duration<double, std::milli> elapsed = end - start;
  double avg_time = elapsed.count() / runs;
  printf("  Kernel 24 Avg Time: %.3f ms | %.1f GFLOPS (includes transpose)\n",
         avg_time,
         (2.0 * rows * cols * inners) / (avg_time * 1e6));
