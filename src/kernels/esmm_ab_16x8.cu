...
// K29: 16×8 A-granularity variant of K25
// A-matrix: 16-row tiles (vs 64 in K25)
...
// A preprocessor with 16-row granularity
template <const int BK, const int TILE_M, const int NUM_THREADS>
__global__ void preprocess_a_16x8(
    int M, int K,
...
__global__ void preprocess_b_16x8(
    int K, int N,
...
// Compute kernel with 16-row A-pattern granularity
// Each warp processes 64 rows but uses 4 different A-patterns (one per 16-row subtile)
template <const int BM, const int BN, const int BK,
          const int WM, const int WN, const int WNITER,
          const int TM, const int TN, const int NUM_THREADS,
          const int A_TILE_M>
__global__ void __launch_bounds__(NUM_THREADS)
esmm_ab_16x8_compute(
    int M, int N, int K,
    const float* __restrict__ A,
    const float* __restrict__ B,
    ...
    // Number of A-pattern tiles per warp (WM / A_TILE_M)
    constexpr uint A_TILES_PER_WARP = WM / A_TILE_M;
    // Number of WMITER iterations per A-tile
    constexpr uint WMITER_PER_A_TILE = WMITER / A_TILES_PER_WARP;
    ...
    // Layout: [warp][a_tile][kBlock]
    __shared__ uint8_t joint_smem[NUM_WARPS * A_TILES_PER_WARP * 1024];

    // Precompute joint patterns with finer A granularity
    {
        const uint globalMBlock = cRow;
        const uint globalNBlock = cCol;
        const int numATilesGlobal = M / A_TILE_M;

        // Total patterns to load: NUM_WARPS * A_TILES_PER_WARP * numKBlocks
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (int dotIdx = 0; dotIdx < BK; ++dotIdx) {
            // Load B registers (same for all A-tiles)
            #pragma unroll
            ...
            // Process each A-tile with its own pattern
            #pragma unroll
            for (int aTile = 0; aTile < A_TILES_PER_WARP; aTile++) {
                // Check if this A-tile has work for this dotIdx
                if (!(joints[aTile] & (1 << dotIdx))) {
                    continue;
                }

                // Load A values for this A-tile's WMITER iterations
                #pragma unroll
                for (uint wSubRowIdx = aTile * WMITER_PER_A_TILE;
                     wSubRowIdx < (aTile + 1) * WMITER_PER_A_TILE; ++wSubRowIdx) {
                    regM[wSubRowIdx * TM + 0] = As[(dotIdx * (BM + 1)) + warpRow * WM +
                        wSubRowIdx * WSUBM + threadRowInWarp * TM + 0];
                }

                // Compute for this A-tile
                #pragma unroll
                for (uint wSubRowIdx = aTile * WMITER_PER_A_TILE;
                     wSubRowIdx < (aTile + 1) * WMITER_PER_A_TILE; ++wSubRowIdx) {
