...
esmm_ab_8x32(
    ...
    const uint8_t* __restrict__ a_patterns,  // [numTileRows × numKBlocks]
    const uint8_t* __restrict__ b_patterns,  // [numWarpCols × numKBlocks]
    int numKBlocks
) {
...

    {
        ...
        for (int i = threadIdx.x; i < totalPatterns; i += NUM_THREADS) {
            ...
            const int wSubRowIdx = remainder / numKBlocks;  // Which 8-row sub-tile (0-3)
            ...
            // A pattern: indexed by 8-row tile
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
            const uint8_t joint = joints[wSubRowIdx];  // Pattern for THIS 8-row sub-tile

            if (joint == 0) continue;  // Skip this 8-row sub-tile entirely
            ...
            for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
                ...
                for (int dotIdx = 0; dotIdx < BK; ++dotIdx) {
                    // Check pattern for THIS specific 8×32 tile
                    if (!(joint & (1 << dotIdx))) {
                        continue;
                    }
