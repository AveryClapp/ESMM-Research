...
esmm_ab_gmem_32(
    ...
    // With NUM_WARPS_M=2 this reads 2 A-pattern rows and 4 B-pattern cols per K-block.
    {
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        // Block-level skip: uniform across all threads
        ...
        // A tile: float2 loads, all 256 threads contribute (rowStrideA = BM)
        #pragma unroll
        ...
        // Warp-level skip: reads A+B patterns from global memory (K25-style)
        // Safe with NUM_WARPS_M=2: uses if() not continue, so __syncthreads below runs always
        uint8_t cur_joint;
        if (laneId == 0) {
        ...
        if (cur_joint != 0) {
            ...
            for (int dotIdx = 0; dotIdx < BK; ++dotIdx) {
                if (!(cur_joint & (1 << dotIdx))) continue;
                ...
                for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
                    ...
                    regN[wSubColIdx * TN + 0] = tmp0.x; regN[wSubColIdx * TN + 1] = tmp0.y;
                    regN[wSubColIdx * TN + 2] = tmp0.z; regN[wSubColIdx * TN + 3] = tmp0.w;
                    regN[wSubColIdx * TN + 4] = tmp1.x; regN[wSubColIdx * TN + 5] = tmp1.y;
                    regN[wSubColIdx * TN + 6] = tmp1.z; regN[wSubColIdx * TN + 7] = tmp1.w;
                }
    ...
    for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
        ...
        for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
            ...
            for (uint resIdxN = 0; resIdxN < TN; resIdxN += 4) {
                float4 tmp;
                const int i = wSubRowIdx * (WNITER * TN) + wSubColIdx * TN + resIdxN;
                tmp.x = threadResults[i + 0]; tmp.y = threadResults[i + 1];
                tmp.z = threadResults[i + 2]; tmp.w = threadResults[i + 3];
                reinterpret_cast<float4*>(
                    &C_sub[threadRowInWarp * N + threadColInWarp * TN + resIdxN])[0] = tmp;
            }
        }
    }
}
