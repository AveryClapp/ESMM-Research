...
__global__ void __launch_bounds__(NUM_THREADS)
    ...
    // Phase 1 & 2: Precompute joint patterns and block-level decisions
    if (threadIdx.x == 0) {
        for (int kb = 0; kb < numKBlocks; kb++) {
            bool any_nonzero = false;

            // Get A pattern for this warp row and k-block
            uint8_t a_pattern = A_blockPatterns[globalWarpRow * numKBlocks + kb];

            // Check all columns this thread block handles
            for (int w = 0; w < WNITER; w++) {
                // Calculate which N block this corresponds to
                const uint globalColBase = cCol * BN + warpCol * WN + w * WSUBN;
                const uint nBlock = globalColBase >> 3;  // Divide by 8 (TN)

                // Get B pattern for this k-block and column
                uint8_t b_pattern = B_blockPatterns[kb * numNBlocks + nBlock];

                // Joint pattern: only compute where BOTH A and B are nonzero
                uint8_t joint = a_pattern & b_pattern;

                // Store for Phase 2
                joint_patterns[kb][w] = joint;
                pattern_popcounts[kb][w] = __popc(joint);
    ...
    // Phase 3: Compute warp-level decisions
    if (laneIdx == 0 && warpIdx < WARPS_PER_BLOCK) {
        for (int kb = 0; kb < numKBlocks; kb++) {
            bool any = false;

            // Check if any column in this warp's range has work
            for (int w = 0; w < WNITER; w++) {
                if (pattern_popcounts[kb][w] > 0) {
                    any = true;
                    break;
                }
            }
    ...
    __syncthreads();  // Ensure all setup is done
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (int sparse_idx = 0; sparse_idx < A_count; ++sparse_idx) {
            ...
            for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
                uint8_t pattern = joint_patterns[kb][wSubColIdx];
                uint8_t count = pattern_popcounts[kb][wSubColIdx];
