...
// ============================================================================
// SIZE=6: dotIdx = 0,1,2,3,4,5
// ============================================================================
template <const int BM, const int BN, const int BK, const int WM, const int WN,
		const int WNITER, const int TM, const int TN, const int NUM_THREADS>
__global__ void __launch_bounds__(NUM_THREADS)
	esmm_unrolled_6(int M, int N, int K, float *A, float *B, float *C) {
	...
	for (int32_t bkIdx = 0; bkIdx < K; bkIdx += BK) {
		...
		// Iterations 0-5 (dotIdx = 0,1,2,3,4,5)
		...
		for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
			regM[wSubRowIdx] = As[(4 * BM) + warpRow * WM + wSubRowIdx * WSUBM + threadRowInWarp * TM];
		}
		for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
			regN[wSubColIdx * TN + 0] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 0];
			regN[wSubColIdx * TN + 1] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 1];
			regN[wSubColIdx * TN + 2] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 2];
			regN[wSubColIdx * TN + 3] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 3];
			regN[wSubColIdx * TN + 4] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 4];
			regN[wSubColIdx * TN + 5] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 5];
			regN[wSubColIdx * TN + 6] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 6];
			regN[wSubColIdx * TN + 7] = Bs[(4 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 7];
		}
		for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
			for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
				threadResults[(wSubRowIdx * TM) * (WNITER * TN) + wSubColIdx * TN + 0] += regM[wSubRowIdx] * regN[wSubColIdx * TN + 0];
		...
		for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
			regM[wSubRowIdx] = As[(5 * BM) + warpRow * WM + wSubRowIdx * WSUBM + threadRowInWarp * TM];
		}
		for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
			regN[wSubColIdx * TN + 0] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 0];
			regN[wSubColIdx * TN + 1] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 1];
			regN[wSubColIdx * TN + 2] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 2];
			regN[wSubColIdx * TN + 3] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 3];
			regN[wSubColIdx * TN + 4] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 4];
			regN[wSubColIdx * TN + 5] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 5];
			regN[wSubColIdx * TN + 6] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 6];
			regN[wSubColIdx * TN + 7] = Bs[(5 * BN) + warpCol * WN + wSubColIdx * WSUBN + threadColInWarp * TN + 7];
		}
		for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
			for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
				threadResults[(wSubRowIdx * TM) * (WNITER * TN) + wSubColIdx * TN + 0] += regM[wSubRowIdx] * regN[wSubColIdx * TN + 0];
